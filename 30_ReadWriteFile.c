...
struct s {
 ...
 int main(){  //main function body
	...
	FILE *fptr;
	int i;
	fptr=fopen("Desktop\\Ayush_101_C_Programs_Repository\\0_ReadWriteFile.txt","wb"); //Opening the file from the location.
	for (i=0;i<1;++i) {
		fflush(stdin);
		printf("Enter Text: ");
	...
	fclose(fptr);
	fptr=fopen("Desktop\\Ayush_101_C_Programs_Repository\\0_ReadWriteFile.txt","wb");
